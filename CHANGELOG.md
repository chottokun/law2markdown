...
### Fixed (修正・不具合対応)
- **ファイル名の OS 禁止文字サニタイズ**: 条文の複数番号指定 XML（`Num="436:448"` や `Num="95_4:95_5"`）から生成されるファイル名に含まれるコロン `:` 等の非互換文字を安全なハイフン `-` に自動変換（Windows や外部ツールでの展開・読み込みエラーを完全に防止）。

### Added (追加)
- ディレクトリ名の極短化・スマート化（原則「法令タイトルのみ」にシンプル化。同一タイトルのバージョン重複時も長いIDではなく `_2`, `_3` のシンプル連番サフィックスを付与して長さを最少化）
